...
export const UploadsProvider: React.FC<{
  children: ReactNode;
}> = ({ children }) => {
  const [uploads, setUploads] = useState<FileUpload[]>(getFiles());

  const updateUploads = useCallback((updater: FileUpload[] | ((prevUploads: FileUpload[]) => FileUpload[])) => {
    setUploads((prevUploads) => {
      const newUploads = typeof updater === "function" ? updater(prevUploads) : updater;
      setFiles({
        val: newUploads
      });
      return newUploads;
...
export const UploadManagerMinimizeProvider: React.FC<{
  ...
  const toggleMinimize = useCallback((props?: { minimize?: boolean }) => {
    setIsMinimized((prev) => {
      ...
      localStorage.setItem("uploadManagerMinimize", newValue ? "true" : "false");
      return newValue;
    });
  }, []);
